...
These are deliberate, already-reviewed trade-offs in `action.yml` and
`.github/workflows/code-review.yml`. Re-reporting them is noise:
...
- `honor_pr_body_decisions` lets an author-controlled PR body suppress findings.
  This repo enables it deliberately (single author, advisory check, forks skipped
  in the caller `if`); do not report the enabled setting here as a vulnerability.
